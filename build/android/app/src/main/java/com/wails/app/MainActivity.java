...
public class MainActivity extends AppCompatActivity {
    ...
    /**
     * Launch the system folder picker using ACTION_OPEN_DOCUMENT_TREE.
     * Returns the selected folder path to JavaScript via a custom event.
     */
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        Log.d(TAG, "onActivityResult: requestCode=" + requestCode + " resultCode=" + resultCode + " data=" + (data != null ? data.getData() : "null"));
        ...
        if (requestCode == FOLDER_PICKER_REQUEST) {
            if (resultCode != RESULT_OK || data == null || data.getData() == null) {
                // User cancelled - emit event with null
                Log.d(TAG, "Folder picker cancelled or no data");
                emitFolderPickerResult(null);
                return;
            }

            // Extract the path from the document tree URI
            new Thread(() -> {
                try {
                    Uri treeUri = data.getData();
                    Log.d(TAG, "Folder picker treeUri: " + treeUri);
                    String path = getPathFromTreeUri(treeUri);
                    Log.d(TAG, "Folder picker resolved path: " + path);
                    emitFolderPickerResult(path);
                } catch (Exception e) {
    ...
    /**
     * Extract a real filesystem path from an ACTION_OPEN_DOCUMENT_TREE URI.
     * For SD cards and external storage, this maps the tree URI to /storage/...
     * For the app's own external files, returns the direct path.
     */
    @Nullable
    private String getPathFromTreeUri(Uri treeUri) {
        ...
        try {
            ...
            // Handle primary storage (internal/emulated)
            // docId is like "primary:data_oms" or just "primary"
            if (docId != null && docId.contains(":")) {
                String[] parts = docId.split(":", 2);
                String volumeId = parts[0];
                String subPath = parts.length > 1 ? parts[1] : "";

                // Map volume IDs to mount points
                String mountPoint = null;
                if ("primary".equals(volumeId)) {
                    mountPoint = android.os.Environment.getExternalStorageDirectory().getAbsolutePath();
                } else {
                    // External SD card or USB - try common mount points
                    File[] storageDirs = ContextCompat.getExternalFilesDirs(this, null);
                    for (File dir : storageDirs) {
                        if (dir != null) {
                            String dirPath = dir.getAbsolutePath();
                            // External dirs are like /storage/XXXX-XXXX/Android/data/pkg/files
                            if (dirPath.contains("/" + volumeId + "/")) {
                                int idx = dirPath.indexOf("/" + volumeId + "/");
                                mountPoint = dirPath.substring(0, idx + volumeId.length() + 1);
                                break;
                            }
                        }
                    }
                    if (mountPoint == null) {
                        mountPoint = "/storage/" + volumeId;
                    }
                }

                // Construct full path: mountPoint + subPath
                if (subPath.isEmpty()) {
                    return mountPoint;
                }
                File fullPath = new File(mountPoint, subPath);
                if (fullPath.exists()) {
                    return fullPath.getAbsolutePath();
                }
                // Directory might not exist yet - return the intended path anyway
                // (the caller will create it)
                return fullPath.getAbsolutePath();
            }

            // Handle just "primary" without subfolder
            if ("primary".equals(docId)) {
                return android.os.Environment.getExternalStorageDirectory().getAbsolutePath();
            }

            // Fallback: try to use DocumentsContract to get the path
            // This works for some common URI formats
            ...
            // Last resort: return the external files directory as a safe default
            File externalDir = getExternalFilesDir(null);
            if (externalDir != null) {
                // Go up to the volume root
                String extPath = externalDir.getAbsolutePath();
            ...
            Log.e(TAG, "getPathFromTreeUri failed", e);
            return null;
        }
    }

    /**
     * Emit a folder picker result using the standard Wails event dispatch.
     * Uses window._wails.dispatchWailsEvent which is set up by the Wails runtime.
     */
    private void emitFolderPickerResult(@Nullable String path) {
        Log.d(TAG, "emitFolderPickerResult: path=" + path);
        final String js;
        if (path != null) {
            String escaped = path.replace("\\", "\\\\").replace("'", "\\'");
            js = "window._wails && window._wails.dispatchWailsEvent({name:'android:folderPickerResult',data:{path:'" + escaped + "'}})";
        } else {
            js = "window._wails && window._wails.dispatchWailsEvent({name:'android:folderPickerResult',data:{cancelled:true}})";
        }
        Log.d(TAG, "emitFolderPickerResult: js=" + js);
        runOnUiThread(() -> {
            if (webView != null) {
