...
public class WailsJSBridge {
    ...
    /**
     * Show the native folder picker on Android.
     * Called from JavaScript: wails.pickFolder()
     * The selected folder path is returned via a custom JS event
     * 'android:folderPickerResult' with detail = path or null.
     */
    @JavascriptInterface
    public void pickFolder() {
        if (DEBUG) Log.d(TAG, "pickFolder called");

        // Run on main thread since we need to start an activity
        ...
        mainHandler.post(() -> {
            ...
            while (ctx != null) {
                if (ctx instanceof MainActivity) {
                    if (DEBUG) Log.d(TAG, "pickFolder: launching folder picker");
                    ((MainActivity) ctx).launchFolderPicker();
                    return;
                }
            ...
            Log.e(TAG, "pickFolder: cannot find MainActivity");
        });
    }
